package com.example.demo.xm.textrank;
...
//            System.out.println(Arrays.toString(scores));
    ...
    {
        //1，有限的确定性算法。这类算法在有限的一段时间内终止。2，有限的非确定算法。这类算法在有限的时间内终止。3，无限的算法。是那些由于没有定义终止定义条件，或输入的数据满足而不终止运行的算法。
        String document = ""+
                "1，有限的确定性算法。这类算法在有限的一段时间内终止。\n" +
                "2，有限的非确定算法。这类算法在有限的时间内终止。\n" +
                "3，无限的算法。是那些由于没有定义终止定义条件，或输入的数据满足而不终止运行的算法。";
        //System.out.println(TextRankSummary.getTopSentenceList(document, 3));
        List<Term> sentenceList = HanLP.segment(document);
        System.out.println(sentenceList);
        String summary = HanLP.getSummary(document,20);//返回的摘要长度
